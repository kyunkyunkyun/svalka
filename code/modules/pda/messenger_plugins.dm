...
/datum/data/pda/messenger_plugin/virus/detonate/user_act(mob/user, obj/item/pda/pda_to_detonate)
	. = ..()
	if(.)
		if(!pda_to_detonate.detonate || pda_to_detonate.GetComponent(/datum/component/uplink))
...
/datum/data/pda/messenger_plugin/virus/frame/user_act(mob/user, obj/item/pda/P)
	...
	if(.)
		...
		var/datum/component/uplink/uplink = P.GetComponent(/datum/component/uplink)
		if(!uplink)
			AddComponent(/datum/component/uplink, _flags = UPLINK_FLAGS_ALWAYS_ACTIVE, _telecrystals = parent_cart.telecrystals)
			P.lock_code = lock_code
		else
			uplink.telecrystals_hidden += uplink.telecrystals // Temporarially hide the PDA's crystals, so you can't steal telecrystals.
			uplink.telecrystals = parent_cart.telecrystals
